...
export default class Pasajeros {
    ...
    static async init() {
        ...
        document.querySelector('#pasajeros-aceptar').addEventListener('click', Pasajeros.#crud)
        document.querySelector('#pasajeros-cerrar').addEventListener('click', Pasajeros.#close)
        return this
    }
    ...
    static async #loadAll() {
        ...
        try {
            const response = await Helpers.fetchData("http://localhost:4567/pasajeros")
    ...
    static async create() {
        ...
        try {
            let response = await Helpers.fetchData(`http://localhost:4567/pasajeros`, {
                method: 'POST',
                body: {
                    identifiacion: document.querySelector('#identificacion').value.trim(),
                    nombre: document.querySelector('#nombres').value.trim(),
            ...
            Helpers.notice(
                '#form-pasajeros #state', 'Sin acceso a la creación de registros', 'danger', e
            )
        }
    }
    ...
    static async retrieve() {
        ...
        try {
            let response = await Helpers.fetchData(
                `http://localhost:4567/pasajeros/id=${identificacion}`
            )

            if (response.message === 'ok') {
                document.querySelector('#nombres').value = response.data.nombre
                document.querySelector('#apellidos').value = response.data.apellido
